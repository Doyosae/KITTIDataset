...
class GetKITTI(object):
    def __init__(self, datapath: str, mode: list, cut = 4):
        """
        KITTI 데이터 폴더의 구조
        ㄴKITTI
    ...
    def search(self):
        all_filename = []
        for yyyy_mm_dd in self.mode: # 각 날짜 별 폴더마다
    ...
    def side_cut(self, filename, cut):
        ...
        """        
        modified_filename = []
        modified_filename = filename[cut: len(filename)-cut]
        return modified_filename



class KITTIMonoDataset(Dataset):
    def __init__(self, datapath, filename, is_training, frame_ids, ext, scale_factor = 1):
        ...
        self.default_scale = [(320, 1024), (192, 640), (96, 320), (48, 160)] # corresponding as scale_factor: 0, 1, 2, 3
        self.scale_list    = [(self.default_scale[self.scale_factor][0]//(2**i), 
                               self.default_scale[self.scale_factor][1]//(2**i)) for i in self.scales]
        print("-- KITTI scaling table")
        print("Scale factor      :  {0}".format(self.scale_factor))
        ...
        for scale, (height, width) in enumerate(self.scale_list):
            self.resize[scale] = Resize(height = int(height), width  = int(width), interpolation = 0)
    ...
    def load_point(self, calib_path, point_path, do_flip):
        ...
        해결 1
        skimage.transform.resize 함수로 해결 
